...
import { counterSwapThroughSmartContractApi, getCurrenciesByChainIdApi, getSwapDetailsByTradeOrOpenTradeIdApi } from "@/service/api";
...
const CounterOfferSwapRoom = () => {
          ...
          try {
            if (tradeId) {
              ...
              const counterSwap: SUI_OpenSwap = {
                ...initSwapData,
                init_address: initSwapData.accept_address,
                accept_address: initSwapData.init_address,
                init_sign: '',
                metadata: {
                  init: {
                    tokens: initSwapData.metadata.accept.tokens
                  },
                  accept: {
                    tokens: initSwapData.metadata.init.tokens
                  }
                }
              };
  ...
  const handleCounterSwapOffer = async () => {
    try {
      ...
      await state.createCounterSwapOffer();
      const createdSwap: any = (swapMode === SUE_SWAP_MODE.OPEN) ? useSwapMarketStore.getState().openMarket.openRoom.swap : useSwapMarketStore.getState().privateMarket.privateRoom.swap;

      if (!createdSwap) {
        throw new Error("Failed to create swap.");
      }

      const userSignMessage = (createdSwap.swap_mode === SUE_SWAP_MODE.OPEN) ? defaults.userSignMessages.open.counterProposal : defaults.userSignMessages.private.counter;
      ...
      const transactionRes = await counterSwapThroughSmartContractApi({
      ...
      console.log("Counter swap error: ", error);
      handleShowNotificationToast(
        "error",
        `Request failed!`,
...
export default CounterOfferSwapRoom;
